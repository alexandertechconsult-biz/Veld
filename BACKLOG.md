...
Stated explicitly because it is easy to assume more protection exists than does.

- **There is no login screen and no account.** Opening the app puts you straight into it.
  The device's own lock screen is the entire access control.
- **Data is stored unencrypted** in IndexedDB. Anyone with access to an unlocked phone,
  or a forensic tool, can read every farm record.
- **The export file is plaintext** and contains the whole farm history. It is the one
  artifact that leaves the device. Deliberate: encryption means a passphrase the farmer
  can forget, which turns a backup into a way to lose records permanently. Documented
  rather than locked.
- **No server, no API, no secrets.** None of the usual web risks apply here: no
  row-level security to get wrong, no service-role routes to audit, no credentials to leak.
- **This inverts the moment a second person logs data.** Sync, a server, real auth and
  row-level security all arrive together at that point. None of them are half-built now.
...
| E2-06 | 12 | E2-03 | As a farmer, I want to correct or remove a livestock record or event I logged wrong | Edit and delete on both the animal or group record and any event; delete asks for confirmation and names what is being removed; an edited event keeps its original date unless the date is what is being corrected | P0 | M |
| E3-01 | 13 | E1-02 | As a farmer, I want to register a field or block so I can track it | Record with name, crop type, optional size | P0 | M |
| E3-02 | 14 | E3-01 | As a farmer, I want to log an activity against a field | Activity has date, type (planting / input / harvest / other) and note; written locally with no network call | P0 | M |
| E3-03 | 15 | E3-02 | As a farmer, I want to see a history of activities per field | Chronological list, most recent first | P0 | S |
| E3-05 | 16 | E3-03 | As a farmer, I want to correct or remove a field or activity I logged wrong | Edit and delete on both the field record and any activity; delete asks for confirmation and names what is being removed | P0 | M |
| E4-01 | 17 | E1-01 | As a farmer, I want to create a task so I don't forget what needs doing | Title required; optional link to a field or animal group, free-text assignee, due date | P0 | S |
| E4-02 | 18 | E4-01 | As a farmer, I want to mark a task done | Single tap; moves to the Done group | P0 | S |
| E4-03 | 19 | E4-02 | As a farmer, I want to edit or delete a task | Edit title, linked field or animal group, assignee and due date; delete asks for confirmation; a task already marked done can be reopened | P0 | S |
| E5-01 | 20 | E1-02 | As a farmer, I want to log a cost or sale so I have a running record | Date, type, amount, optional enterprise link, note; one screen, four fields | P0 | S |
| E7-01 | 21 | E2-02, E3-02, E4-01, E5-01 | As a farmer, I want a home screen showing recent activity across all modules | Last five entries, mixed types, most recent first | P0 | M |
| E7-02 | 22 | E7-01 | As a farmer, I want a fast way to log something without hunting through menus | "+" opens a quick-add sheet with four options; any log completable in under three taps from home | P0 | M |
| E1-05 | 23 | E1-01 | As a farmer, I want to correct my farm name or details | Edit the farm profile from Settings; no delete, since the farm is the root record | P0 | S |
| E1-03 | 24 | E7-02 | As a farmer, I want a guided first-run flow so I'm not staring at an empty app | First launch walks farm name, then first enterprise, then done | P0 | M |
| **GATE-1** | 25 | every Phase 1 ticket, Seq 7 through 24 | Human checkpoint | Barrett logs a real day's activity end to end on his phone and confirms the three-tap rule holds before demo work starts | — | — |

### Phase 2: Demo readiness

| ID | Seq | Depends | User Story | Acceptance Criteria | Priority | Est. |
|---|---|---|---|---|---|---|
| E8-01 | 26 | GATE-1 | As Barrett, I want a pre-populated demo farm so I can show the concept without the farmer's real data | Seed data loads a sample enterprise, several animals and fields, and a few days of realistic events | P0 | S |
| E8-02 | 27 | E8-01, E0-05 | As Barrett, I want the demo to work on my phone with no setup friction in front of the farmer | Installable PWA loads demo data on first open with one tap; airplane-mode logging works, per the Section 12 demo flow | P0 | S |
| E6-05 | 28 | GATE-1, E0-03 | As a farmer, I want to export my data so a lost phone doesn't mean lost records | Export writes every entity to a single file; import restores it to an empty install; round-trip covered by a test | P0 | M |
| **GATE-2** | 29 | E8-01, E8-02, E6-05 | Human checkpoint | Barrett runs the full Section 12 demo flow, airplane mode included, before it goes in front of the farmer | — | — |
...
| ID | Seq | Depends | User Story | Acceptance Criteria | Priority | Est. |
|---|---|---|---|---|---|---|
| E7-03 | 30 | GATE-2 | As a farmer, I want today's and overdue tasks on the home screen | Home surfaces open tasks due today or overdue | P1 | S |
| E5-02 | 31 | GATE-2, E5-01 | As a farmer, I want a running total per enterprise and overall | Sum display only, no charts | P1 | S |
| E5-03 | 32 | GATE-2, E5-01 | As a farmer, I want to edit or delete a transaction I logged in error | Edit and delete from the transaction list | P1 | S |
| E2-04 | 33 | GATE-2, E2-02 | As a farmer, I want to attach a photo to an event, for example an injury | Photo stored in IndexedDB alongside the event and included in export | P1 | M |
| E3-04 | 34 | GATE-2, E3-02 | As a farmer, I want to attach a photo to an activity, for example crop damage | Photo stored in IndexedDB alongside the activity and included in export | P1 | M |
| E2-05 | 35 | GATE-2, E2-01 | As a farmer, I want to search and filter my livestock list | Search by name or tag; filter by species | P1 | S |
| E1-04 | 36 | GATE-2, E1-02 | As a farmer, I want to edit or remove an enterprise later | Edit and delete from Settings; delete requires confirmation | P1 | S |
| E6-06 | 37 | GATE-2, E6-05 | As a farmer, I want to be warned before an import overwrites what's there | Import shows what will be replaced and requires confirmation | P1 | S |
